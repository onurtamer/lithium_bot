...
class LithiumBot(commands.Bot):
    ...
    async def on_app_command_error(
        ...
        if interaction.response.is_done():
            await interaction.followup.send(
                f"⚠️ An error occurred: `{error_msg}`", ephemeral=True
            )
        else:
    ...
    async def on_ready(self):
        ...
        if not self.intents.message_content or not self.intents.members:
            logger.error(
                "CRITICAL: Missing required intents (Message Content or Members)!"
            )
            ...
            try:
                ...
                for guild in self.guilds:
                    try:
                        ...
                        await r.set(
                            f"guild:stats:{guild.id}:members", json.dumps(stats), ex=300
                        )
                        ...
                        logger.warning(
                            f"Failed to update stats for guild {guild.id}: {e}"
                        )
                ...
                try:
                    ...
                    if data.get("action") == "DIAGNOSTIC":
                        ...
                        if guild:
                            ...
                            diag_res["permissions"] = perms.administrator or (
                                perms.manage_guild
                            ...
                            diag_res["log_access"] = (
                                perms.view_audit_log and perms.embed_links
                            )
